...
/// Thrown by `LiveServer.liveSave` when the server file no longer matches the expectation.
...
protocol LiveServer: AnyObject, Sendable {
    ...
    /// Downloads `item` over `local`, replacing it in one rename. `interactive` puts it on the
    /// lane the user is waiting on.
...
/// nothing needs a lock. What each pass does is `LiveDecision`'s answer, from the facts it
/// gathers: the working copy's stamp, a digest when the stamp moved, and the server's file.
///
/// One per app, owned by the hub, so the one watcher and the records outlive any connection.
actor LiveSync {
    ...
    init(store: Store, watches: Bool = true) {
        ...
        let (loaded, gone) = Self.load(store.liveFiles(), store: store)
        entries = loaded
        notices = gone
    }

    /// Records from their rows. A working copy edited while Transfer was closed, or before its
    /// server logged in, is dirty at once, so Quit and Remove Server count it. A synced copy
    /// untouched for a day expires; its last activity is the later of the file's and its
    /// folder's mtime, since a download gives the file the server's older time.
    private static func load(_ rows: [LiveRow], store: Store) -> ([LiveFileID: Entry], [ConnectionID: [String]]) {
        ...
        for row in rows {
            ...
            var change = LiveDecision.localChange(entry.state, stamp, digest: nil)
            if change == .needDigest { change = LiveDecision.localChange(entry.state, stamp, digest: digest(of: local) ?? "unreadable") }
            ...
            if !entry.state.dirty, entry.conflict == nil, change != .changed,
               Date().timeIntervalSince(max(stamp.mtime, folderTime)) > Self.expiry {
                store.deleteLive(row.id)
    ...
    /// A connection logged in: its files are looked at again, and passes that waited for the
    /// server run.
    func connected(_ connection: ConnectionID, server: any LiveServer) {
        ...
        if var worker = workers[connection] {
            for id in worker.waitingForServer {
                // The "Waiting for the server" row goes; the pass shows what is really left.
                if let entry = entries[id] { server.liveEmit(.operation(Self.operation(entry, state: .succeeded))) }
                worker.due[id] = .now
            }
            worker.waitingForServer.removeAll()
    ...
    /// Only the current connection object counts: the hub may have replaced an older one, which
    /// can still disconnect on its own.
    func disconnected(_ connection: ConnectionID, server: any LiveServer) {
        guard servers[connection]?.server === server else { return }
    ...
    func files(on connection: ConnectionID) -> [LiveFile] {
        entries.values.filter { $0.connection == connection }.map {
            LiveFile(id: $0.id, path: $0.path, dirty: $0.state.dirty, paused: $0.state.paused, conflict: $0.state.conflict, uploading: $0.uploading)
        }
        .sorted { $0.path.display < $1.path.display }
    }

    /// Files with edits the server does not have, including ones no pass has seen yet. Nil counts
    /// every server's.
    func unsyncedCount(on connection: ConnectionID? = nil) -> Int {
        entries.values.filter { entry in
            ...
            if entry.state.dirty { return true }
            guard let stamp = Self.stamp(entry.local) else { return false }
            var change = LiveDecision.localChange(entry.state, stamp, digest: nil)
            if change == .needDigest { change = LiveDecision.localChange(entry.state, stamp, digest: Self.digest(of: entry.local) ?? "unreadable") }
            return change == .changed
        }.count
    }

    /// The working copy for `path`, downloaded now, or brought up to date when the server moved on
    ...
    func setPaused(_ path: RemotePath, on connection: ConnectionID, paused: Bool) {
        ...
        if paused {
            emit(entry, .operation(Self.operation(entry, state: .paused)))
        } else {
            // Resume and Retry: the row goes, and a pass shows what is left to do.
            emit(entry, .operation(Self.operation(entry, state: .succeeded)))
    ...
    func discard(_ path: RemotePath, on connection: ConnectionID, force: Bool) async throws {
        try await perform(on: connection) {
            guard let id = await self.find(path, on: connection) else { return }
            try await self.discardNow(id, force: force)
        }
    }

    /// A rename on the server of `source`, run by `perform`. When Live files lie under it, it runs
    /// between their passes and their records follow it; otherwise it runs at once.
    func rename(_ source: RemotePath, to destination: RemotePath, on connection: ConnectionID, perform rename: @escaping @Sendable () async throws -> Void) async throws {
        guard entries.values.contains(where: { $0.connection == connection && $0.path.isInside(source) }) else {
            return try await rename()
        }
        try await perform(on: connection) {
            try await rename()
            await self.moved(source, to: destination, on: connection)
        }
    }
    ...
    func remove(_ path: RemotePath, on connection: ConnectionID, perform remove: @escaping @Sendable () async throws -> Void) async throws {
        guard entries.values.contains(where: { $0.connection == connection && $0.path.isInside(path) }) else {
            return try await remove()
        }
        try await perform(on: connection) {
            try await remove()
            await self.removed(path, on: connection)
        }
    }
    ...
    /// Only the working copy's own path matters; editor swap files, our temps, and the
    /// "(server)" copy beside it are not the file.
    private func handle(_ signal: LiveWatcher.Signal, from watcher: LiveWatcher) {
        ...
        case .changed(let paths):
            for path in paths {
                guard let parts = watcher.components(of: path), parts.count >= 2,
                      let uuid = UUID(uuidString: parts[1]) else { continue }
                let id = LiveFileID(rawValue: uuid)
                guard let entry = entries[id], parts.count == 2 || parts[2] == entry.local.lastPathComponent else { continue }
                localChanged(id)
            }
        }
    }
    ...
    private func park(_ id: LiveFileID) {
        ...
        emit(entry, .operation(Self.operation(entry, state: .queued, message: "Waiting for the server")))
    }

    private func startWorker(_ connection: ConnectionID) {
        if workers[connection] == nil { workers[connection] = Worker() }
        guard workers[connection]?.loop == nil else { return }
        workers[connection]?.loop = Task { await self.run(connection) }
    }
    ...
    private func run(_ connection: ConnectionID) async {
        while !Task.isCancelled, let worker = workers[connection] {
            ...
            if let next = worker.due.filter({ $0.value <= now }).min(by: { $0.value < $1.value })?.key {
    ...
    private func idle(_ connection: ConnectionID) async {
        ...
        workers[connection]?.timer?.cancel()
        workers[connection]?.timer = nil
    }

    private func poke(_ connection: ConnectionID) {
        guard let wake = workers[connection]?.wake else { return }
        workers[connection]?.wake = nil
        wake.resume()
    }
    ...
    private func perform<T: Sendable>(on connection: ConnectionID, _ body: @escaping @Sendable () async throws -> T) async throws -> T {
        try await withCheckedThrowingContinuation { (done: CheckedContinuation<T, Error>) in
            ...
            workers[connection]?.commands.append(Command(
                run: {
                    do { done.resume(returning: try await body()) } catch { done.resume(throwing: error) }
                },
    ...
    private func pass(_ id: LiveFileID) async {
        ...
        while let entry = entries[id] {
            ...
            case .none, .refreshLocal:
                return
            case .markClean:
                change(id) { $0.state.dirty = false }
                return
            case .markDirty:
                change(id) { $0.state.dirty = true }
                return
            case .restamp:
                update(id) {
            ...
            case .recheckMissing:
                ...
                schedule(id, after: Self.missingGrace)
                return
            case .failMissing:
                emit(entry, .operation(Self.operation(entry, state: .failed, message: "The working copy disappeared before its edits were uploaded")))
                return
            case .forget:
                forget(id)
                emit(entry, .liveChanged)
            ...
            case .adopt(let print):
                ...
                return
            case .failRetryable(let reason):
                return retry(id, reason: reason)
            case .conflict(let kind):
                return await raiseConflict(id, kind: kind, item: item)
    ...
    private func lookup(_ id: LiveFileID) async -> (RemoteItem?, LiveServerFact)? {
        ...
        guard ready.contains(entry.connection), let server = liveServer(for: entry.connection) else {
            park(id)
            return nil
        }
        do {
            let item = try await server.liveLookup(entry.path)
            guard let item else { return (nil, .missing) }
    ...
    private func retry(_ id: LiveFileID, reason: String) {
        guard let entry = entries[id] else { return }
        guard ready.contains(entry.connection) else { return park(id) }
        if let delay = RetryPolicy.delay(afterAttempt: entry.attempts) {
            entries[id]?.attempts += 1
            schedule(id, after: .seconds(delay))
        } else {
            emit(entry, .operation(Self.operation(entry, state: .failed, message: reason)))
        }
    }
    ...
    private func save(_ id: LiveFileID, expecting: ServerExpectation, via server: any LiveServer) async -> SaveOutcome {
        ...
        let snapshot: URL
        do {
            snapshot = try Self.snapshot(of: entry.local)
        } catch {
            emit(entry, .operation(Self.operation(entry, state: .failed, message: error.localizedDescription)))
            return .failed
        }
        ...
        guard let digest = Self.digest(of: snapshot) else {
            emit(entry, .operation(Self.operation(entry, state: .failed, message: "Could not read \(entry.name)")))
            return .failed
        }
        let progress: @Sendable (TransferProgress) -> Void = { [entry] progress in
            server.liveEmit(.operation(Self.operation(entry, state: .active, progress: progress)))
        }
        do {
            let print = try await server.liveSave(snapshot, to: entry.path, expecting: expecting, progress: progress)
            ...
            emit(saved, .operation(Self.operation(saved, state: saved.state.paused ? .paused : .succeeded)))
    ...
    private func raiseConflict(_ id: LiveFileID, kind: LiveConflictKind, item: RemoteItem?) async {
        ...
        case .removed:
            message = "Removed from the server"
        case .notAFile:
            message = "Replaced on the server by something that is not a file"
        }
        emit(conflicted, .operation(Self.operation(conflicted, state: .failed, message: message)))
    ...
    private func openNow(_ path: RemotePath, on connection: ConnectionID) async throws -> URL {
        ...
        if let id = find(path, on: connection), let entry = entries[id] {
            if let stamp = Self.stamp(entry.local) {
                var local = LiveLocal.present(stamp, digest: nil)
                var action = LiveDecision.decide(entry.state, local: local, server: .file(print), intent: .open)
                if action == .needDigest {
                    local = .present(stamp, digest: Self.digest(of: entry.local))
                    action = LiveDecision.decide(entry.state, local: local, server: .file(print), intent: .open)
                }
                ...
                } else if LiveDecision.localChange(entry.state, stamp, digest: nil) != .same {
                    look(id)
                }
                return entry.local
            }
    ...
    private func resolveNow(_ path: RemotePath, on connection: ConnectionID, choice: LiveConflictChoice) async throws {
        ...
        case .compare:
            return
        case .keepLocal:
            let expecting: ServerExpectation
            switch entry.conflict {
            case .changed(let print): expecting = .file(print)
            case .removed: expecting = .absent
            case .notAFile: throw TransferError.typeMismatch(path.display)
            case nil: expecting = entry.state.base.map(ServerExpectation.file) ?? .absent
            }
            ...
            case .unstable:
                throw TransferError.failed("\(entry.name) is still being written")
            case .retry(let reason):
                throw TransferError.failed(reason)
            case .failed:
                throw TransferError.failed("Could not upload \(entry.name)")
            }
        case .keepRemote:
        ...
        case .keepBoth:
            ...
            var name = "\(entry.name) (from this Mac)"
            var n = 2
            while names.contains(name) {
                name = "\(entry.name) (from this Mac \(n))"
                n += 1
            }
        ...
        emit(resolved, .operation(Self.operation(resolved, state: .succeeded)))
    ...
    /// Keep Remote: the server's file replaces the working copy. When the server has none, the
    /// mapping goes.
    private func takeServerCopy(_ id: LiveFileID, via server: any LiveServer) async throws {
        guard let entry = entries[id] else { return }
        guard let item = try await server.liveLookup(entry.path), item.kind == .file, let print = Fingerprint(item: item) else {
            forget(id)
            try? FileManager.default.removeItem(at: entry.folder)
            emit(entry, .liveChanged)
            return
    ...
    private func compare(_ path: RemotePath, on connection: ConnectionID) async throws {
        ...
        if !FileManager.default.fileExists(atPath: entry.serverCopy.path) {
            guard let server = liveServer(for: connection), let item = try await server.liveLookup(entry.path), item.kind == .file else {
        ...
        process.arguments = [entry.local.path, entry.serverCopy.path]
        try process.run()
    }
    ...
    private func removed(_ path: RemotePath, on connection: ConnectionID) {
        for entry in entries.values where entry.connection == connection && entry.path.isInside(path) {
            forget(entry.id)
            try? FileManager.default.removeItem(at: entry.folder)
            emit(entry, .liveChanged)
        }
    ...
    /// `update`, then tells the windows.
    private func change(_ id: LiveFileID, _ body: (inout Entry) -> Void) {
        guard let entry = update(id, body) else { return }
        emit(entry, .liveChanged)
    }
    ...
    private static func entry(from row: LiveRow, local: URL) -> Entry {
        ...
        var synced: LiveStamp?
        if let size = row.syncedSize, let mtime = row.syncedMtime { synced = LiveStamp(size: size, mtime: Date(timeIntervalSinceReferenceDate: mtime)) }
        var conflict: LiveConflictKind?
        switch row.conflict {
        case "changed":
            if let size = row.conflictSize, let mtime = row.conflictMtime { conflict = .changed(Fingerprint(kind: .file, size: size, mtime: mtime)) }
        case "removed": conflict = .removed
        case "notAFile": conflict = .notAFile
        default: break
        }
        let state = LiveState(base: base, synced: synced, syncedDigest: row.syncedDigest, dirty: row.dirty, paused: row.paused, conflict: conflict != nil)
    ...
    private static func row(_ entry: Entry) -> LiveRow {
        var row = LiveRow(
            ...
            path: entry.path,
            baseSize: entry.state.base?.size,
            baseMtime: entry.state.base?.mtime,
            localPath: entry.local.path,
            dirty: entry.state.dirty,
            paused: entry.state.paused,
            syncedSize: entry.state.synced?.size,
            syncedMtime: entry.state.synced?.mtime.timeIntervalSinceReferenceDate,
            syncedDigest: entry.state.syncedDigest
        )
        switch entry.conflict {
        case .changed(let print):
            row.conflict = "changed"
            row.conflictSize = print.size
            row.conflictMtime = print.mtime
        case .removed: row.conflict = "removed"
        case .notAFile: row.conflict = "notAFile"
        case nil: break
        }
        return row
    }

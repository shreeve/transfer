...
final class ChromeController: NSSplitViewController {
    ...
    override func viewDidAppear() {
        ...
        scheduleSnapshotIfRequested(window)
    }

    /// Debug aid: with TRANSFER_SNAPSHOT set to a file path, the whole window frame including the
    /// toolbar is rendered there a moment after it appears. Screen capture needs permissions this
    /// process may not have; rendering the frame view does not.
    private func scheduleSnapshotIfRequested(_ window: NSWindow) {
        guard let path = ProcessInfo.processInfo.environment["TRANSFER_SNAPSHOT"], !path.isEmpty else { return }
        let delay = Double(ProcessInfo.processInfo.environment["TRANSFER_SNAPSHOT_DELAY"] ?? "") ?? 2
        DispatchQueue.main.asyncAfter(deadline: .now() + delay) {
            ...
            func walk(_ v: NSView, _ depth: Int) {
                ...
                let layerBG = v.layer?.backgroundColor.map { "\($0)" } ?? "-"
                let border = v.layer.map { "bw=\($0.borderWidth)" } ?? ""
                dump += String(repeating: "  ", count: depth) + "\(type(of: v)) frame=\(NSStringFromRect(f)) hidden=\(v.isHidden) alpha=\(v.alphaValue) bg=\(layerBG) \(border) sublayers=\(v.layer?.sublayers?.count ?? 0)\n"
                for sub in v.subviews { walk(sub, depth + 1) }
            }
            walk(frame, 0)
            dump += "\n---- window: contentView safeArea=\(window.contentView!.safeAreaInsets) contentLayoutRect=\(window.contentLayoutRect) frame=\(window.frame)\n"
            if let desc = frame.perform(Selector(("_subtreeDescription")))?.takeUnretainedValue() as? String {
                dump += "\n---- _subtreeDescription\n" + desc
            }
            dump += "\n---- state: window.titlebarSeparatorStyle=\(window.titlebarSeparatorStyle.rawValue) toolbar.showsBaselineSeparator=\(window.toolbar?.showsBaselineSeparator ?? false) items=\(self.splitViewItems.map { $0.titlebarSeparatorStyle.rawValue }) styleMask=\(window.styleMask.rawValue) toolbarStyle=\(window.toolbarStyle.rawValue)\n"
            @MainActor func lineVisible() -> (Bool, Bool) {
                guard let rep = frame.bitmapImageRepForCachingDisplay(in: frame.bounds) else { return (false, false) }
                frame.cacheDisplay(in: frame.bounds, to: rep)
                func dark(_ x: Int) -> Bool {
                    guard let c = rep.colorAt(x: x, y: 104)?.usingColorSpace(.sRGB) else { return false }
                    return (0.299 * c.redComponent + 0.587 * c.greenComponent + 0.114 * c.blueComponent) < 0.92
                }
                return (dark(120), dark(1400))
            }
            dump += "baseline: \(lineVisible())\n"
            var all: [NSView] = []
            @MainActor func collect(_ v: NSView) { all.append(v); for s in v.subviews { collect(s) } }
            collect(frame)
            for v in all where !v.isHidden {
                v.isHidden = true
                let r = lineVisible()
                v.isHidden = false
                if !r.0 || !r.1 {
                    dump += "hiding \(type(of: v)) \(NSStringFromRect(v.convert(v.bounds, to: nil))) -> \(r)\n"
                }
            }
            @MainActor func introspect(_ obj: NSObject, filter: ((String) -> Bool)? = nil) {
                var cls: AnyClass? = type(of: obj)
                dump += "\n#### \(type(of: obj))\n"
                while let c = cls, c != NSView.self, c != NSObject.self {
                    dump += "class \(c)\n"
                    var count: UInt32 = 0
                    if let ivars = class_copyIvarList(c, &count) {
                        for i in 0..<Int(count) {
                            ...
                            var value = "?"
                            if enc.hasPrefix("@") || enc == "B" || enc == "c" || enc == "q" || enc == "Q" || enc == "d" || enc == "i" || enc == "I" {
                                value = "\(obj.value(forKey: name).map { "\($0)" } ?? "nil")"
                            }
                            dump += "  ivar \(name) [\(enc)] = \(value.prefix(160))\n"
                        }
                        free(ivars)
                    }
                    var mcount: UInt32 = 0
                    if let methods = class_copyMethodList(c, &mcount) {
                        var names: [String] = []
                        for i in 0..<Int(mcount) { names.append(NSStringFromSelector(method_getName(methods[i]))) }
                        free(methods)
                        let shown = filter == nil ? names : names.filter { filter!($0) }
                        dump += "  methods: \(shown.sorted().joined(separator: " "))\n"
                    }
                    cls = class_getSuperclass(c)
                }
            }
            let interesting: (String) -> Bool = { n in
                let l = n.lowercased()
                return l.contains("pocket") || l.contains("edge") || l.contains("separator") || l.contains("divider") || l.contains("line") || l.contains("style") || l.contains("safearea")
            }
            for v in all {
                let name = "\(type(of: v))"
                if name == "NSScrollPocket" { introspect(v) }
                if name == "ListCoreScrollView" || name == "NSTitlebarBackgroundView" || name == "NSTitlebarView" || name == "NSThemeFrame" { introspect(v, filter: interesting) }
            }
            try? dump.write(toFile: path + ".txt", atomically: true, encoding: .utf8)
            // EXPERIMENTS
            struct Exp { let name: String; let apply: @MainActor () -> Void; let revert: @MainActor () -> Void }
            var exps: [Exp] = []
            let items = self.splitViewItems
            let detail = items[1], sidebar = items[0]
            for style: NSTitlebarSeparatorStyle in [.automatic, .line, .shadow] {
                exps.append(Exp(name: "detailItem.titlebarSeparatorStyle=\(style.rawValue)", apply: { detail.titlebarSeparatorStyle = style }, revert: { detail.titlebarSeparatorStyle = .none }))
            }
            for style: NSTitlebarSeparatorStyle in [.automatic, .line, .shadow] {
                exps.append(Exp(name: "window.titlebarSeparatorStyle=\(style.rawValue)", apply: { window.titlebarSeparatorStyle = style }, revert: { window.titlebarSeparatorStyle = .none }))
            }
            exps.append(Exp(name: "detail.allowsFullHeightLayout=true", apply: { detail.allowsFullHeightLayout = true }, revert: { detail.allowsFullHeightLayout = false }))
            exps.append(Exp(name: "sidebar.allowsFullHeightLayout=false", apply: { sidebar.allowsFullHeightLayout = false }, revert: { sidebar.allowsFullHeightLayout = true }))
            exps.append(Exp(name: "no fullSizeContentView", apply: { window.styleMask.remove(.fullSizeContentView) }, revert: { window.styleMask.insert(.fullSizeContentView) }))
            exps.append(Exp(name: "toolbar.showsBaselineSeparator=true", apply: { window.toolbar?.showsBaselineSeparator = true }, revert: { window.toolbar?.showsBaselineSeparator = false }))
            exps.append(Exp(name: "toolbarStyle=unifiedCompact", apply: { window.toolbarStyle = .unifiedCompact }, revert: { window.toolbarStyle = .unified }))
            exps.append(Exp(name: "toolbarStyle=automatic", apply: { window.toolbarStyle = .automatic }, revert: { window.toolbarStyle = .unified }))
            exps.append(Exp(name: "detail.automaticallyAdjustsSafeAreaInsets=false", apply: { detail.automaticallyAdjustsSafeAreaInsets = false }, revert: { detail.automaticallyAdjustsSafeAreaInsets = true }))
            exps.append(Exp(name: "detailHost.safeAreaRegions=[]", apply: { self.detailHost?.safeAreaRegions = [] }, revert: { self.detailHost?.safeAreaRegions = .all }))
            exps.append(Exp(name: "sidebarHost.safeAreaRegions=[]", apply: { self.sidebarHost?.safeAreaRegions = [] }, revert: { self.sidebarHost?.safeAreaRegions = .all }))
            exps.append(Exp(name: "detailHost.sizingOptions=default", apply: { self.detailHost?.sizingOptions = [.minSize, .intrinsicContentSize] }, revert: { self.detailHost?.sizingOptions = [] }))
            @MainActor func pockets() -> [NSView] {
                var out: [NSView] = []
                func walk(_ v: NSView) { if "\(type(of: v))" == "NSScrollPocket" { out.append(v) }; for s in v.subviews { walk(s) } }
                walk(frame); return out
            }
            exps.append(Exp(name: "PRIVATE pockets.allowsSeparator=false", apply: { pockets().forEach { $0.setValue(false, forKey: "allowsSeparator") } }, revert: { pockets().forEach { $0.setValue(true, forKey: "allowsSeparator") } }))
            exps.append(Exp(name: "PRIVATE pockets._showSeparatorsOnHardPocket=false", apply: { pockets().forEach { $0.setValue(false, forKey: "_showSeparatorsOnHardPocket") } }, revert: { pockets().forEach { $0.setValue(true, forKey: "_showSeparatorsOnHardPocket") } }))
            exps.append(Exp(name: "PRIVATE pockets.style=1", apply: { pockets().forEach { $0.setValue(1, forKey: "style") } }, revert: { pockets().forEach { $0.setValue(0, forKey: "style") } }))
            var results = "sidebar variant: \(ProcessInfo.processInfo.environment["TRANSFER_EXP_SIDEBAR"] ?? "-")\nbaseline: \(lineVisible())\n"
            @MainActor func describePockets() -> String {
                pockets().map { p in
                    let bg = p.superview.map { "\(type(of: $0)) style=\($0.value(forKey: "_style") ?? "?")" } ?? ""
                    return "[\(NSStringFromRect(p.convert(p.bounds, to: nil))) style=\(p.value(forKey: "style") ?? "?") sep=\(p.value(forKey: "allowsSeparator") ?? "?") hardSep=\(p.value(forKey: "_showSeparatorsOnHardPocket") ?? "?") in \(bg)]"
                }.joined(separator: " ")
            }
            results += "pockets: \(describePockets())\n"
            @MainActor func run(_ i: Int) {
                guard i < exps.count else {
                    try? results.write(toFile: path + ".exp.txt", atomically: true, encoding: .utf8)
                    return
                }
                let e = exps[i]
                e.apply()
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
                    MainActor.assumeIsolated {
                        results += "\(e.name): \(lineVisible()) \(describePockets())\n"
                        e.revert()
                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.4) { MainActor.assumeIsolated { run(i + 1) } }
                    }
                }
            }
            run(0)

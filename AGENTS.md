...
- A move removes a source only after `MoveCheck` finds that this move wrote a complete copy: an entry the destination held before the move began never counts, and a file needs an equal size and a known, equal time. Finder originals go to the Trash.

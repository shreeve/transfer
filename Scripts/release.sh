...
# the app reads from the latest release (SUFeedURL). The bundle is ad-hoc signed, as DuckTable
# ships: no Developer ID, no notarization. The feed is signed with the EdDSA key in the login
# keychain, the private half of SUPublicEDKey.
#
...
CONFIG=release Scripts/package-app.sh >/dev/null
